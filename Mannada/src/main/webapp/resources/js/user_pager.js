...
function getNotice(page, query) {
	...
	fetch(url)
        ...
        .then(result => {
            ...
            if(page >= result.notice_pager.last || window.location.pathname == '/'){
...
function makeNotice(element) {
    ...
    notice_item.forEach(item => {
        ...
        if(name==title){
        	...
        	a.href = "/notice/detail/" + element[href];
...
function getQnA(page, query) {
	...
	fetch(url)
        ...
        .then(result => {
            ...
            if(page >= result.qna_pager.last || window.location.pathname == '/'){
...
function makeQnA(element) {
    ...
    qna_item.forEach(item => {
        ...
        if(name=="title"){
        	const a = document.createElement("a");
        	a.href = "/qna/detail/" + element[href];
